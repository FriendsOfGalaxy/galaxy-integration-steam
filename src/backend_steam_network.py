...
class SteamNetworkBackend(BackendInterface):
    ...
    async def _authenticate_with_stored_credentials(self, stored_credentials) -> Union[NextStep, Authentication]:
        ...
        # Save migrated credentials back to Galaxy if migration occurred
        if self._user_info_cache.is_initialized():
            migrated_creds = self._user_info_cache.to_dict()
            if migrated_creds != stored_credentials:  # Migration occurred
                logger.info("Credentials were migrated, saving new format to Galaxy")
                self._store_credentials(migrated_creds)
    ...
    async def get_user_presence(self, user_id: str, context: Any) -> UserPresence:
        ...
        if user_info is None:
            raise UnknownError(
                f"User {user_id} not in friend list (plugin only supports fetching presence for friends)"
            )
